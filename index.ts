...
function elideCube<Measures extends Keys, Dimensions extends Keys>(def: {
  name: string;
  measures: Measures;
  dimensions: Dimensions;
...
> = {
  ...
  >;
  finalize(): {
    measures: { [K in InputMeasures]: Measures[K] };
    dimensions: { [K in InputDimensions]: Dimensions[K] };
  };
};

function consume<Measures extends Keys, Dimensions extends Keys>(
